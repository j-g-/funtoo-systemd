...
# $Header: /var/cvsroot/gentoo-x86/net-libs/nodejs/nodejs-0.12.2-r1.ebuild,v 1.1 2015/04/11 17:39:36 mgorny Exp $
...
src_configure() {
	...
	if use npm ; then
		pushd ${S}/deps/npm/
		./configure --globalconfig=/etc/node/npmrc
		popd
	fi
}
...
src_install() {
	...
	use npm && insinto /etc/node/ && newins ${FILESDIR}/npmrc npmrc
